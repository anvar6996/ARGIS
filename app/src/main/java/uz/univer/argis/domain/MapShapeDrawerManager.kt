...
class MapShapeDrawerManager(
    ...
    fun drawCurrentMarkersPolygon() {
        ...
        map.addPolygon(polygonOptions).apply {
            currentPolygon = this
            if (!isDragging)
                fillColor = resources.getColor(selectedType.color)
    ...
    fun removeLastMarker() {
        when {
            ...
            shapeModels.isNotEmpty() && markers.isEmpty() -> {
                ...
                selectedType =
                    types.firstOrNull { it == lastShapeModel.type } ?: types.first()
    ...
    fun loadToCache() {
        val cache = ShapeModelForJsonList(
            shapeModelJsonFormats = shapeModels.map { it.toJsonFormat() }
        )
    ...
    fun loadFromCache() {
        ...
        data.shapeModelJsonFormats.forEach { shapeModelJsonFormat ->
            ...
            val polygon = addPolygon(
                shapeModelJsonFormat.points.map { it.toLatLng() },
                shapeModelJsonFormat.color
            )
            shapeModels.add(
                ShapeModel(
                    markers,
                    polygon,
                    Type(shapeModelJsonFormat.name, shapeModelJsonFormat.color)
                )
            )
        }
    }
